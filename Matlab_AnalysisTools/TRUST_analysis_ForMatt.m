%% Script for Matt, analysing TRUST data
...
cd(['/Users/caitlino/Desktop/DataBackup/MR_PHYSICS_', subject]);
addpath('/Users/caitlino/Desktop/DPHIL_YEAR_2/MATLAB');

filename_TRUST = dir('*pasl_TRUST*.dat'); % Find all filenames with this name
a = 1; %take first (or 2nd or 3rd etc) of those files
[coils_TRUST, phase_TRUST, complex_data_TRUST] = EPI_phasecor_coil_sens(filename_TRUST(a).name); %Perform EPI phase correction and coil sensitivity calc using Adaptive Combine 
im_data_TRUST = squeeze(sum(complex_data_TRUST(1:32, :,:,:), 1)); % combine all the coils
    
im_data_TRUST = reshape_coil_data(im_data_TRUST, averages, TE); % reshape function
