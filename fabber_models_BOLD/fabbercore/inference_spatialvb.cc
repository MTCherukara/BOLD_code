...
void SpatialVariationalBayes::Initialize(FwdModel* fwd_model, ArgsType& args)
{
  ...
  assert(maxPrecisionIncreasePerIteration > 1 || maxPrecisionIncreasePerIteration == -1);
	...
	// ---------------------------------------------------------------
	// ---------------------------------------------------------------
	// NEW STUFF - parse input arguments for 'allow-bad-voxels'
...
cout << "Just to be sure, haltOnBadVoxel = " << haltOnBadVoxel << endl;
...
// UPDATE SPATIAL SHRINKAGE PRIOR PARAMETERS
 ...
 if (shrinkageType != '-' && (!isFirstIteration || updateSpatialPriorOnFirstIteration))
	...
	for (int k = 1; k <= Nparams; k++) // loop through parameters
	  {
	...
	if (shrinkageType == 'S')
	  {
		...
		if (v != i)
		 ...
		 //if (conv->NeedRevert())
		 //{
			 // cout << "Reverting to saved values" << endl;
...
// ---------------------------------------------------------------
// ---------------------------------------------------------------
// NEW STUFF - starting the try loop here
try
{
// ---------------------------------------------------------------
// ---------------------------------------------------------------
	...
	if (1)
	  ...
	  Tracer_Plus tr("SpatialVariationalBayes::DoCalculations - new spatial prior calculations, try and catch");
...
// ---------------------------------------------------------------
// ---------------------------------------------------------------
// NEW STUFF - end TRY, catches

} // TRY
catch (const overflow_error& e)
{
	cout << "Haha, caught one! (overflow)" << endl;
	LOG_ERR("    Went infinite!  Reason:" << endl << "      " << e.what() << endl);
	// if (haltOnBadVoxel) throw;
	LOG_ERR("    Resetting, and going on to the next stage." << endl);
	fwdPosteriorVox[v-1] = fwdPosteriorVoxSave[v-1];
	fwdPriorVox[v-1] = fwdPriorVoxSave[v-1];
	*noiseVox[v-1] = *noiseVoxSave[v-1];
}    
...
{
	cout << "Haha, caught one! (exception)" << endl;
...
// ---------------------------------------------------------------
    ...
    if (useSimultaneousEvidenceOptimization) // not using this 
      {
    ...
    else if (useFullEvidenceOptimization) // not using this either 
      {
...
//	assert(!useCovarianceMarginalsRatherThanPrecisions); 
  ...
  // ---------------------------------------------------------------
	// ---------------------------------------------------------------
	// NEW STUFF - if at first you don't succeed:
	...
	// */
	  ...
	  // ---------------------------------------------------------------
        ...
        {
					cout << "Haha, caught one later! (overflow)" << endl;
	        LOG_ERR("    Went infinite!  Reason:" << endl << "      " << e.what() << endl);
	        if (haltOnBadVoxel) throw;
        ...
        {
					cout << "Haha, caught one later! (exception)" << endl;
					LOG_ERR("    NEWMAT Exception in this voxel:\n" << Exception::what() << endl);
					if (haltOnBadVoxel) throw;
					LOG_ERR("    Going on to the next voxel." << endl);
					fwdPosteriorVox[v-1] = fwdPosteriorVoxSave[v-1];
        ...
        {
					cout << "Haha, caught one later! (...)" << endl;
					LOG_ERR("    Other exception caught in main calculation loop!!\n");
					if (haltOnBadVoxel) throw;
					LOG_ERR("    Going on to the next voxel." << endl);
					fwdPosteriorVox[v-1] = fwdPosteriorVoxSave[v-1];
			...
			// after resetting manually, save again manually?
			fwdPosteriorVoxSave[v-1] = fwdPosteriorVox[v-1];
			fwdPriorVoxSave[v-1] = fwdPriorVox[v-1];
			*noiseVoxSave[v-1] = *noiseVox[v-1];
  ...
  // Phew!
	...
	if (conv->NeedRevert())
	{
		cout << "End of main loop, reverting values" << endl;
  ...
  for (int v = 1; v <= Nvoxels; v++)
    {
      ...
      delete fwdPosteriorWithoutPrior.at(v-1);
			// ------------------------------------------------------------
			// ------------------------------------------------------------
			// NEW STUFF - deleting noiseVoxSave 
			delete noiseVoxSave[v-1];
			// ------------------------------------------------------------
			// ------------------------------------------------------------
    }
} // Do Calculations 
